...
resource "openstack_networking_secgroup_v2" "timescale_pprof_security_group" {
  name        = "timescale_pprof_security_group"
  description = "timescale security group"
}
...
resource "openstack_networking_secgroup_rule_v2" "pprof" {
  ...
  port_range_max    = 8080
  remote_ip_prefix  = each.value
  security_group_id = openstack_networking_secgroup_v2.timescale_pprof_security_group.id
}
